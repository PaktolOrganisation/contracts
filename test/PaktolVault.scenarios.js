...
/**
 * EIP-712 signed deposit for restricted vaults (REQUIRES_AUTH = true).
 *
 * @param {object} eure         - EURe contract instance
 * @param {object} vault        - PaktolVault instance (restricted)
 * @param {object} backendSigner - Hardhat signer that acts as backend wallet
 * @param {object} user         - Hardhat signer making the deposit
 * @param {bigint} amount       - Amount of EURe to deposit
 */
async function depositWithAuth(eure, vault, backendSigner, user, amount) {
  ...
  const sig = await backendSigner.signTypedData(domain, types, value);

  await eure.connect(user).approve(await vault.getAddress(), amount);
  return vault.connect(user).depositWithAuth(amount, user.address, deadline, sig);
}

// ─────────────────────── FIXTURE ─────────────────────────────────────────────

async function deployFixture() {
  // signer = backend wallet that authorizes restricted-vault deposits
  const [deployer, owner, treasury, guardian, harvester, signer, alice, bob, charlie, attacker] =
  ...
  // Standard vault — open deposits, 0.5% fee, 3.5% cap
  const vaultStd = await PaktolVault.deploy({
    asset:        await eure.getAddress(),
    name:         "Paktol Standard",
    symbol:       "pkEUR-S",
    owner:        owner.address,
  ...
  // Paktol subscription vault — requires backend auth, no fee, 5% cap
  const vaultPkt = await PaktolVault.deploy({
    asset:        await eure.getAddress(),
    name:         "Paktol Subscription",
    symbol:       "pkEUR-P",
    owner:        owner.address,
    treasury:     treasury.address,
    capBps:       CAP_PKT,
  ...
  return {
    ...
    deployer, owner, treasury, guardian, harvester, signer,
...
describe("PaktolVault — Business Scenarios", function () {
  ...
  describe("Scenario 5: Paktol subscription plan (AAVE 6%)", function () {
    it("No fee deducted, net yield capped at 5%, surplus goes to treasury", async function () {
      const { eure, pool, aToken, vaultPkt, harvester, treasury, signer, alice } = await deployFixture();

      // Subscription vault requires backend auth — use depositWithAuth
      await depositWithAuth(eure, vaultPkt, signer, alice, HUNDRED_K_EURE);
    ...
    it("Paktol plan user gets more net yield than Standard plan at same AAVE APY", async function () {
      const { eure, pool, aToken, vaultStd, vaultPkt, harvester, signer, alice, bob } = await deployFixture();

      // Alice → Standard (open deposit), Bob → Paktol (requires auth)
      await eure.connect(alice).approve(await vaultStd.getAddress(), HUNDRED_K_EURE);
      await vaultStd.connect(alice).deposit(HUNDRED_K_EURE, alice.address);

      await depositWithAuth(eure, vaultPkt, signer, bob, HUNDRED_K_EURE);
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SCENARIO 7: Emergency exit from AAVE
  // ═══════════════════════════════════════════════════════════════════════════

  describe("Scenario 7: Emergency exit from AAVE", function () {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SCENARIO 8: Guardian pause flow
  // ═══════════════════════════════════════════════════════════════════════════

  describe("Scenario 8: Guardian pause and owner unpause", function () {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SCENARIO 9: Ownable2Step — safe ownership transfer
  // ═══════════════════════════════════════════════════════════════════════════

  describe("Scenario 9: Ownable2Step", function () {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SCENARIO 10: Inflation attack resistance
  // ═══════════════════════════════════════════════════════════════════════════

  describe("Scenario 10: Inflation attack resistance", function () {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SCENARIO 11: Full year — 12 monthly harvests
  // ═══════════════════════════════════════════════════════════════════════════

  describe("Scenario 11: Full year simulation — yield cap enforced over 12 months", function () {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SCENARIO 12: depositWithPermit (EIP-2612 gasless deposit)
  // ═══════════════════════════════════════════════════════════════════════════

  describe("Scenario 12: depositWithPermit — gasless EIP-2612 deposit", function () {
    ...
    it("depositWithPermit is blocked on restricted vault", async function () {
      const { eure, vaultPkt, alice } = await deployFixture();
      ...
      const sig = await alice.signTypedData(domain, types, {
        owner: alice.address, spender: await vaultPkt.getAddress(),
        value: amount, nonce, deadline,
      });
      const { v, r, s } = ethers.Signature.from(sig);

      await expect(
        vaultPkt.connect(alice).depositWithPermit(amount, alice.address, deadline, v, r, s)
      ).to.be.revertedWithCustomError(vaultPkt, "UseDepositWithAuth");
    });
  });

  // ═══════════════════════════════════════════════════════════════════════════
  // SCENARIO 13: depositWithAuth — subscription-gated vault
  // ═══════════════════════════════════════════════════════════════════════════

  describe("Scenario 13: depositWithAuth — Stripe-gated Paktol vault", function () {
    it("Subscriber deposits with valid backend signature", async function () {
      const { eure, vaultPkt, signer, alice } = await deployFixture();

      await depositWithAuth(eure, vaultPkt, signer, alice, THOUSAND_EURE);

      expect(await vaultPkt.balanceOf(alice.address)).to.be.gt(0n);
      expect(await vaultPkt.nonces(alice.address)).to.equal(1n);
    });

    it("Direct deposit() on restricted vault reverts with UseDepositWithAuth", async function () {
      const { eure, vaultPkt, alice } = await deployFixture();

      await eure.connect(alice).approve(await vaultPkt.getAddress(), THOUSAND_EURE);
      await expect(
        vaultPkt.connect(alice).deposit(THOUSAND_EURE, alice.address)
      ).to.be.revertedWithCustomError(vaultPkt, "UseDepositWithAuth");
    });

    it("Expired signature reverts with SignatureExpired", async function () {
      const { eure, vaultPkt, signer, alice } = await deployFixture();

      const deadline = BigInt(await time.latest()) - 1n; // already expired
      const nonce    = await vaultPkt.nonces(alice.address);
      const chainId  = (await ethers.provider.getNetwork()).chainId;

      const sig = await signer.signTypedData(
        { name: await vaultPkt.name(), version: "1", chainId, verifyingContract: await vaultPkt.getAddress() },
        { DepositAuth: [
          { name: "sender",   type: "address" },
          { name: "assets",   type: "uint256" },
          { name: "receiver", type: "address" },
        ...
        { sender: alice.address, assets: THOUSAND_EURE, receiver: alice.address, deadline, nonce }
      );

      await eure.connect(alice).approve(await vaultPkt.getAddress(), THOUSAND_EURE);
      await expect(
        vaultPkt.connect(alice).depositWithAuth(THOUSAND_EURE, alice.address, deadline, sig)
      ).to.be.revertedWithCustomError(vaultPkt, "SignatureExpired");
    });

    it("Wrong signer (not backend) reverts with InvalidSignature", async function () {
      const { eure, vaultPkt, alice, bob } = await deployFixture();

      // Bob signs instead of the backend signer
      const deadline = BigInt(await time.latest()) + 3600n;
      const nonce    = await vaultPkt.nonces(alice.address);
      const chainId  = (await ethers.provider.getNetwork()).chainId;

      const sig = await bob.signTypedData(
        { name: await vaultPkt.name(), version: "1", chainId, verifyingContract: await vaultPkt.getAddress() },
        { DepositAuth: [
          { name: "sender",   type: "address" },
          { name: "assets",   type: "uint256" },
      ...
      await expect(
        vaultPkt.connect(alice).depositWithAuth(THOUSAND_EURE, alice.address, deadline, sig)
      ).to.be.revertedWithCustomError(vaultPkt, "InvalidSignature");
    });

    it("Replaying the same signature reverts — nonce consumed", async function () {
      const { eure, vaultPkt, signer, alice } = await deployFixture();
      ...
      await eure.connect(alice).approve(await vaultPkt.getAddress(), THOUSAND_EURE * 2n);

      // First call succeeds
      await vaultPkt.connect(alice).depositWithAuth(THOUSAND_EURE, alice.address, deadline, sig);

      // Replay with same sig — nonce mismatch → InvalidSignature
      await expect(
        vaultPkt.connect(alice).depositWithAuth(THOUSAND_EURE, alice.address, deadline, sig)
      ).to.be.revertedWithCustomError(vaultPkt, "InvalidSignature");
    });
